...
export class AddUser {
  ...
  async add(userName: string, userPassword: string): Promise<void> {
    ...
    try {
      const responseAdd = await this.cognitoClient.send(new AdminCreateUserCommand(params));
    ...
    try {
      const responsePass = await this.cognitoClient.send(new AdminSetUserPasswordCommand(paramsPerm));
